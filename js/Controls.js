export class Controls {
	...
	setupTouchUI() {
		...
		css.textContent = `
			...
			.drive-buttons { position: absolute; inset: 0; pointer-events: none; z-index: 3; }
			.drive-btn { position: absolute; bottom: max(18px, env(safe-area-inset-bottom)); width: 84px; height: 84px; border: 2px solid rgba(255,255,255,0.35); border-radius: 50%; background: rgba(0,0,0,0.35); color: white; font: 700 14px/1 sans-serif; touch-action: none; -webkit-user-select: none; user-select: none; pointer-events: auto; }
			.drive-btn.brake { left: max(18px, env(safe-area-inset-left)); }
			.drive-btn.gas { right: max(18px, env(safe-area-inset-right)); }
			.drive-btn.active { background: rgba(255,255,255,0.28); transform: scale(0.96); }
			.gyro-controls { position: absolute; left: 50%; top: max(14px, env(safe-area-inset-top)); transform: translateX(-50%); display: flex; gap: 8px; pointer-events: auto; z-index: 3; }
		...
		const bindHoldButton = ( button, property ) => {
			const press = ( e ) => {
				e.preventDefault(); e.stopPropagation(); this[ property ] = true; button.classList.add( 'active' );
				if ( button.setPointerCapture ) button.setPointerCapture( e.pointerId );
			};
			const release = ( e ) => { e.preventDefault(); e.stopPropagation(); this[ property ] = false; button.classList.remove( 'active' ); };
			button.addEventListener( 'pointerdown', press );
			button.addEventListener( 'pointerup', release );
			button.addEventListener( 'pointercancel', release );
			button.addEventListener( 'lostpointercapture', () => { this[ property ] = false; button.classList.remove( 'active' ); } );
		};
		...
		gyroBtn.addEventListener( 'click', async ( e ) => {
			...
			if ( this.gyroEnabled ) { this.calibrateGyro(); gyroBtn.textContent = 'GYRO ON'; return; }
			const enabled = await this.enableGyro();
			if ( enabled ) {
				gyroBtn.textContent = 'GYRO ON'; gyroBtn.classList.add( 'enabled' ); calibrateBtn.disabled = false; base.style.display = 'none';
			} else gyroBtn.textContent = 'GYRO BLOCKED';
		} );
		calibrateBtn.addEventListener( 'click', ( e ) => { e.preventDefault(); e.stopPropagation(); this.calibrateGyro(); } );

		const steerRange = 40;
		steerZone.addEventListener( 'pointerdown', ( e ) => {
			if ( this.gyroEnabled || this.steerPointerId !== null ) return;
			steerZone.setPointerCapture( e.pointerId ); this.steerPointerId = e.pointerId;
			this.steerStartX = e.clientX; this.steerStartY = e.clientY; this.touchActive = true; this.touchDirX = 0; this.touchDirY = 0;
			base.style.left = `${ e.clientX }px`; base.style.top = `${ e.clientY }px`; base.style.display = 'block';
		} );
		steerZone.addEventListener( 'pointermove', ( e ) => {
			...
			this.touchDirX = dx; this.touchDirY = dy;
			knob.style.transform = `translate(${ this.touchDirX * 60 }px, ${ this.touchDirY * 60 }px)`;
		} );
		const endSteer = ( e ) => {
			if ( e.pointerId !== this.steerPointerId ) return;
			this.steerPointerId = null; this.touchActive = false; this.touchDirX = 0; this.touchDirY = 0; knob.style.transform = ''; base.style.display = 'none';
		};
	...
	update() {
		...
		for ( const gp of gamepads ) {
			if ( ! gp ) continue;
			const stickX1 = gp.axes[ 0 ] || 0, stickX2 = gp.axes[ 2 ] || 0;
			...
			if ( triggerR > 0.1 || triggerL > 0.1 ) z = triggerR - triggerL;
			else {
				const stickY1 = gp.axes[ 1 ] || 0, stickY2 = gp.axes[ 3 ] || 0;
				const stickY = Math.abs( stickY1 ) > Math.abs( stickY2 ) ? stickY1 : stickY2;
				if ( Math.abs( stickY ) > 0.15 ) z = - stickY;
			}
		...
		if ( this.gyroEnabled ) {
			...
			const jx = this.touchDirX, jy = this.touchDirY;
			const mag = Math.sqrt( jx * jx + jy * jy );
			if ( mag > 0.15 ) { x = ( jx + jy ) * Math.SQRT1_2 / mag; z = ( - jx + jy ) * Math.SQRT1_2 / mag; }
		}
		if ( this.btnUp && ! this.btnDown ) z = 1;
		else if ( this.btnDown && ! this.btnUp ) z = -1;
		else if ( this.btnUp && this.btnDown ) z = 0;
