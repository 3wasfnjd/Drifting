...
export class Vehicle {
	...
	updateRoadRunner( dt ) {
		...
		for ( const node of [ bodyTail, legs, eyes, head ] ) {
			...
			if ( node === eyes && node.geometry ) {
				node.geometry.computeBoundingBox();
				const bb = node.geometry.boundingBox;
				node.userData.roadRunnerBlinkHalfHeight = bb ? ( bb.max.y - bb.min.y ) * node.scale.y * 0.5 : 0.02;
			}
		}
		...
		// Soft launch-shaped loop: elongated and curved like the cartoon leg trail, not a tilted circle.
		...
		// Blink keeps the top edge anchored, so the eye closes upward instead of sinking into the head.
		if ( eyes?.userData.roadRunnerBase ) {
			...
			if ( this.roadRunnerBlinkTimer <= 0 && this.roadRunnerBlinkTime <= 0 ) {
				this.roadRunnerBlinkTime = 0.12;
				this.roadRunnerBlinkTimer = 1.8 + Math.random() * 3.5;
			}
			if ( this.roadRunnerBlinkTime > 0 ) this.roadRunnerBlinkTime -= dt;
			const p = this.roadRunnerBlinkTime > 0 ? 1 - this.roadRunnerBlinkTime / 0.12 : 0;
			const blink = this.roadRunnerBlinkTime > 0 ? Math.sin( p * Math.PI ) : 0;
			const close = 1 - blink * 0.78;
			const halfHeight = eyes.userData.roadRunnerBlinkHalfHeight || 0.02;
			eyes.scale.x = base.scale.x;
			eyes.scale.z = base.scale.z;
			eyes.scale.y = THREE.MathUtils.lerp( eyes.scale.y, base.scale.y * close, Math.min( 1, dt * 48 ) );
			eyes.position.y = THREE.MathUtils.lerp( eyes.position.y, base.position.y + halfHeight * ( 1 - close ), Math.min( 1, dt * 48 ) );
		}
	}
}
