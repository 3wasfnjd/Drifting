...
export class ARManager {
	...
	_setupControllers() {
		for ( let i = 0; i < 2; i ++ ) {
			...
			this.scene.add( grip );
			try { grip.add( this.controllerModelFactory.createControllerModel( grip ) ); }
			catch ( e ) { console.warn( 'Controller model failed to load (non-fatal):', e ); }
		}
	}
	...
	update( frame, dt ) {
		...
		// While the mode-selection menu is open, placement is disabled.
		// Do not request reference spaces or hit-test sources until a mode is chosen.
		if ( ! this.placementEnabled && ! this.placed ) return;
		try {
			const refSpace = this.renderer.xr.getReferenceSpace();
			this._ensureHitTestSource();
			if ( this.placementEnabled && ! this.placed ) this._updatePlacement( frame, refSpace, dt );
		} catch ( e ) {
	...
	_updatePlacement( frame, refSpace, dt ) {
		...
		if ( this.hasHit && this._triggerPressedEdge() ) this._confirmPlacement();
	}
	...
	_confirmPlacement() {
		this.placed = true;
		this.previewGroup.visible = false;
		if ( this.world && this.buildFreeRoamFloor ) this._buildFreeRoamFloor();
