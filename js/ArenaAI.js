...
function groundVisual(vehicle,group,groundY){group.updateMatrixWorld(true);const box=new THREE.Box3().setFromObject(group);if(Number.isFinite(box.min.y)){vehicle.visualGroundCorrection=(groundY-box.min.y);group.position.y+=vehicle.visualGroundCorrection}}
...
export function createArenaAI({world,parent,models,center=new THREE.Vector3(),scale=1,count=3}){
 ...
 for(let i=0;i<count;i++){
  ...
  const group=vehicle.init(models[keys[i%keys.length]]);if(scale!==1)group.scale.setScalar(scale);parent.add(group);vehicle.container.position.set(pos[0],center.y,pos[2]);groundVisual(vehicle,group,center.y);
  drivers.push({vehicle,center:center.clone(),scale,aiState:'CRUISING',stateTimer:1.5+Math.random()*2.5,target:center.clone(),sampleTimer:0,stuckStrikes:0,groundY:center.y});
 }
 return drivers;
}
...
export function updateArenaAI(drivers,dt,arenaRadius){
 for(const d of drivers){const v=d.vehicle,limit=arenaRadius*d.scale*.90,wander=arenaRadius*d.scale*.72;d.sampleTimer+=dt;
  ...
  v.update(dt,input);
  // Vehicle positions the collider center correctly; this correction only grounds the rendered model.
  if(v.visualGroundCorrection)v.container.position.y+=v.visualGroundCorrection;
 }
}
